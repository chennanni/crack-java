...
public class ReentrantReadWriteLockReadNotFullyFair {
    ...
    public static void read(int milisToSleep) {
    ...
    public static void writeAndCreateRead(int milisToSleep) {
        System.out.println(Thread.currentThread().getName() + " 开始尝试获取写锁");
        writeLock.lock();
        try {
            System.out.println(Thread.currentThread().getName() + " !!! 获取写锁，开始执行");
            ...
            new Thread(() -> {
                ...
                for (int i = 0; i < subThreadNumber; i++) {
                    subThreads[i] = new Thread(() -> read(1), "   Sub Thread + " + i);
                }
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> writeAndCreateRead(10*threadNumber), "Zero Thread").start();
